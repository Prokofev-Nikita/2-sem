...
bool validateIp(const char *ip) {
    char ip_copy[16];  // Создаем копию IP-адреса для безопасного разбора
    strcpy(ip_copy, ip);
    ...
    for (int i = 0; i < 4; i++) {
        if (ip_parts[i] > 255) return false;
    }
    ...
    if (mask < 0 || mask > 32) return false;  // Проверяем допустимый диапазон маски
